	...
	elseif($get == 'data_fasilitas'){
		if(isset($_POST['edit_fasilitas'])){
			...
			if(!$gambar){
				...
				unlink('img/FasilitasPantai/'. $gambar_lama);
				if(!$gambar){
					return error;
				}
			}
			...
			$edit = mysqli_query($db, "UPDATE fasilitas SET 
				id_fasilitas     = '$id',
			...
			if($edit){
				echo "<script>alert('Data berhasil diedit!'); document.location.href='kelolaFasilitas.php'</script>";
				// echo mysqli_error($koneksi);
			}else{
				echo mysqli_error($db);
			}
			...
			echo "<script>alert('Data gagal diedit!'); document.location.href='kelolaFasilitas.php'</script>";
		}
	}
}
